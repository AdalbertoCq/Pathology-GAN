...
nki_path = '/media/adalberto/Disk2/Cancer_TMA_Generative/dataset/nki/he/new/patches_h224_w224'
vgh_path = '/media/adalberto/Disk2/Cancer_TMA_Generative/dataset/vgh/he/new/patches_h224_w224'
...
nki_train_l = read_hdf5(hdf_path_nki_train, 'train_labels')
nki_test = read_hdf5(hdf_path_nki_test, 'test_img')
nki_test_l = read_hdf5(hdf_path_nki_test, 'test_labels')
...
vgh_train_l = read_hdf5(hdf_path_vgh_train, 'train_labels')
vgh_test = read_hdf5(hdf_path_vgh_test, 'test_img')
vgh_test_l = read_hdf5(hdf_path_vgh_test, 'test_labels')
...
# Train HDF5.
hdf_path_nki_vgh_train = '/media/adalberto/Disk2/Cancer_TMA_Generative/dataset/nki_vgh/he/new/patches_h224_w224/hdf5_nki_vgh_he_train.h5'
hdf5_nki_vgh_train_file = h5py.File(hdf_path_nki_vgh_train, mode='w')
img_db_shape = [total_train] + list(nki_train.shape[1:])
labels_db_shape = [total_train] + list(nki_train_l.shape[1:])
train_img_storage = hdf5_nki_vgh_train_file.create_dataset(name='train_img', shape=img_db_shape, dtype=np.uint8)
train_labels_storage = hdf5_nki_vgh_train_file.create_dataset(name='train_labels', shape=labels_db_shape, dtype=np.float32)

ind = 0
for i in nki_train_ran_ind[:n_nki_comb_train_1]:
    train_img_storage[ind] = nki_train[i]
    train_labels_storage[ind] = nki_train_l[i]
    ind += 1
for i in vgh_train_ran_ind[:n_vgh_comb_train_1]:
    train_img_storage[ind] = vgh_train[i]
    train_labels_storage[ind] = vgh_train_l[i]
    ind += 1
for i in nki_test_ran_ind[:n_nki_comb_train_2]:
    train_img_storage[ind] = nki_test[i]
    train_labels_storage[ind] = nki_test_l[i]
    ind += 1
for i in vgh_test_ran_ind[:n_vgh_comb_train_2]:
    train_img_storage[ind] = vgh_test[i]
    train_labels_storage[ind] = vgh_test_l[i]
    ind += 1

    
# Train HDF5.
hdf_path_nki_vgh_test = '/media/adalberto/Disk2/Cancer_TMA_Generative/dataset/nki_vgh/he/new/patches_h224_w224/hdf5_nki_vgh_he_test.h5'
hdf5_nki_vgh_test_file = h5py.File(hdf_path_nki_vgh_test, mode='w')
img_db_shape = [total_test] + list(nki_train.shape[1:])
labels_db_shape = [total_test] + list(nki_train_l.shape[1:])
test_img_storage = hdf5_nki_vgh_test_file.create_dataset(name='test_img', shape=img_db_shape, dtype=np.uint8)
test_labels_storage = hdf5_nki_vgh_test_file.create_dataset(name='test_labels', shape=labels_db_shape, dtype=np.float32)
ind = 0
for i in nki_train_ran_ind[n_nki_comb_train_1:]:
    test_img_storage[ind] = nki_train[i]
    test_labels_storage[ind] = nki_train_l[i]
    ind += 1
for i in vgh_train_ran_ind[n_vgh_comb_train_1:]:
    test_img_storage[ind] = vgh_train[i]
    test_labels_storage[ind] = vgh_train_l[i]
    ind += 1
for i in nki_test_ran_ind[n_nki_comb_train_2:]:
    test_img_storage[ind] = nki_test[i]
    test_labels_storage[ind] = nki_test_l[i]
    ind += 1
for i in vgh_test_ran_ind[n_vgh_comb_train_2:]:
    test_img_storage[ind] = vgh_test[i]
    test_labels_storage[ind] = vgh_test_l[i]
    ind += 1