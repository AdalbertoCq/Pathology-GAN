...
def optimizer(beta_1, loss_gen, loss_dis, loss_type, learning_rate_input, beta_2=None, clipping=None, display=True):
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        if 'wasserstein distance' in loss_type and 'gradient penalty' in loss_type:
            train_discriminator = tf.train.AdamOptimizer(learning_rate_input, beta_1, beta_2).minimize(loss_dis, var_list=discriminator_variables)
            train_generator = tf.train.AdamOptimizer(learning_rate_input, beta_1, beta_2).minimize(loss_gen, var_list=generator_variables)
        ...
        #TODO Fix this for RMSProp.
            ...
            train_generator = tf.train.AdamOptimizer(learning_rate=learning_rate_input, beta1=beta_1).minimize(loss_gen, var_list=generator_variables)
            train_discriminator = tf.train.AdamOptimizer(learning_rate=learning_rate_input, beta1=beta_1).minimize(loss_dis, var_list=discriminator_variables) 
