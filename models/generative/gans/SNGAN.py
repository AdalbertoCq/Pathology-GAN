...
class SNGAN:
	def __init__(self,
				...
				model_name='SNGAN'           # Name to give to the model.
				):
	...
	def generator(self, z_input, reuse, is_train):
		with tf.variable_scope('generator', reuse=reuse):
			...
			# Conv.
			# net = convolutional(inputs=net, output_channels=16, filter_size=5, stride=1, padding='SAME', conv_type='convolutional', spectral=True, power_iterations=self.power_iterations)
			# net = tf.layers.batch_normalization(inputs=net, training=is_train)
			# net = leakyReLU(net, self.alpha)
			# Shape = (None, 112, 112, 16)

			# Conv.
			# net = convolutional(inputs=net, output_channels=16, filter_size=2, stride=2, padding='SAME', conv_type='transpose', spectral=True, power_iterations=self.power_iterations)
