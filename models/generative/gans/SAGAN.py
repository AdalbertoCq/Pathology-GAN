...
class DCGAN:
	...
	def discriminator(self, images, reuse):
		with tf.variable_scope('discriminator', reuse=reuse):
			...
			net = self.attention_block(net, 1)
			# Shape = (None, 28, 28, 64)
			...
			net = leakyReLU(net, self.alpha)
			net = self.attention_block(net, 2)
			...
			if self.use_bn: net = tf.layers.batch_normalization(inputs=net, training=True)
			net = leakyReLU(net, self.alpha)
			net = self.attention_block(net, 3)
			# Shape = (None, 7, 7, 256)
	...
	def generator(self, z_input, reuse, is_train):
		with tf.variable_scope('generator', reuse=reuse):
			...
			# TODO: Not sure about this attention block.
			net = self.attention_block(net, 4)
			# Shape = (None, 56, 56, 3)
	...
	# Pending, original paper sugests to use hinge loss.
	def loss(self):
		# Discriminator loss.
