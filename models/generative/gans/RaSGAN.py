...
class RaSGAN:
	...
	def generator(self, z_input, reuse, is_train):
	    with tf.variable_scope('generator', reuse=reuse):
	        ...
	        # Dense.
	        net = tf.layers.dense(inputs=z_input, units=1048, activation=None)
	        ...
	        # Conv.
	        net = tf.layers.conv2d(inputs=net, filters=128, kernel_size=(4,4), strides=(1,1), padding='same', kernel_initializer=tf.contrib.layers.xavier_initializer())
	        net = tf.layers.batch_normalization(inputs=net, training=is_train)
	        net = leakyReLU(net, self.alpha)
	        # Shape = (None, 14, 14, 128)
	        ...
	        # Conv.
	        net = tf.layers.conv2d(inputs=net, filters=64, kernel_size=(4,4), strides=(1,1), padding='same', kernel_initializer=tf.contrib.layers.xavier_initializer())
	        net = tf.layers.batch_normalization(inputs=net, training=is_train)
	        net = leakyReLU(net, self.alpha)
	        # Shape = (None, 14, 14, 128)
	        ...
	        net = tf.layers.batch_normalization(inputs=net, training=is_train)
	        net = leakyReLU(net, self.alpha)
	        # Shape = (None, 56, 56, 128)
	        
	        logits = tf.layers.conv2d_transpose(inputs=net, filters=self.image_channels, kernel_size=(2,2), strides=(2,2), padding='same', kernel_initializer=tf.contrib.layers.xavier_initializer(), name='logits')
