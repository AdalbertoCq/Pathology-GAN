...
class WGAN:
	def __init__(self, data, z_dim, use_bn, alpha, beta_1, beta_2, n_critic, clipping, learning_rate, model_name):
	...
	def loss(self):
		# Discriminator loss.
		loss_dis_real = tf.reduce_mean(self.logits_real)
		loss_dis_fake = tf.reduce_mean(self.logits_fake)
	...
	def optimization(self):
		...
		# According to the paper, RMSprop works better to stabilize training.
		with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
	        ...
	        dis_weight_clipping = [value.assign(tf.clip_by_value(value, -self.clipping, self.clipping)) for value in discriminator_variables]
